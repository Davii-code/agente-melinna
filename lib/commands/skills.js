...
export async function skillsInstall(cwd, names, options = {}) {
  ...
  if (options.all) {
    ...
    if (evidence.length > 0) {
      console.log(chalk.cyan(`Stack detectada: ${evidence.map((e) => `${e.tag} (${e.reason})`).join(", ")}`));
    } else {
...
export async function skillsUpdate() {
  ...
  for (const entry of REGISTRY) {
    ...
    const code = await runInherit("git", ["pull", "--ff-only", "--quiet"], { cwd: dest });
    if (code !== 0) {
...
export function skillsList(root, cwd, options = {}) {
  ...
  if (options.detect) {
    ...
    console.log(
      evidence.length > 0
        ? chalk.dim(`  stack: ${evidence.map((e) => `${e.tag} (${e.reason})`).join(", ")}`)
