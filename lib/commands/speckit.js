...
export async function speckit(cwd, featureName, options) {
  // A validação da entrada vem antes de qualquer outra checagem: o valor vira
  // argumento de um processo filho, e recusá-lo cedo dá a mensagem de erro certa
  // (em vez de esconder o problema atrás de "specify não encontrado").
  if (options.integration && !INTEGRATION_CHOICES.includes(options.integration)) {
    console.log(chalk.red(`Integração "${options.integration}" não é suportada.`));
