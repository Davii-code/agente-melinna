 ...
 */
function resolveSkills(root, cwd, options, profile) {
  ...
  const selectOpts = profile.skillLimit ? { limit: profile.skillLimit } : {};
  return { skills: selectSkills(available, tags, selectOpts), evidence };
}
...
export async function task(root, cwd, description, options) {
  ...
  try {
    ...
    selected = resolveSkills(root, cwd, options, profile);
  } catch (err) {
