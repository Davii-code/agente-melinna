...
export function vaultConfig() {
  ...
  return {
    ...
    // Gravação automática é opt-in: pedir a gravação ao fim de cada resposta
    // interrompe a conversa. O caminho normal é `/melinna-salvar`.
    autoSave: vault.autoSave === true,
  };
}

/** Liga o vault numa raiz, criando a estrutura. */
export function enableVault(root, { cooldownMinutes, autoSave } = {}) {
...
export function writeProjectNote(vaultPath, identity, update, meta = {}) {
  ...
  // O índice reflete o estado atual, então é regenerado junto com a nota.
...
/**
 * Reescreve o índice do vault: uma nota que lista e linka todos os projetos.
 *
 * Sem índice, `projetos/` vira uma pilha plana e o agente precisa varrer o
 * diretório para descobrir o que existe — o modo de falha mais citado de vaults
 * de segundo cérebro. Um mapa único resolve com uma leitura.
 *
 * @param {string} vaultPath
 * @returns {{ path: string, count: number }}
 */
export function writeIndexNote(vaultPath) {
  ...
  const lines = [
    ...
    "# Projetos",
    "",
    "Mapa dos projetos que a Melinna acompanha. Regenerado a cada gravação —",
    "edições feitas à mão aqui são perdidas.",
    "",
  ];

  if (projects.length === 0) {
    lines.push("_(nenhum projeto registrado ainda)_", "");
  } else {
    for (const id of projects) {
      const note = readProjectNote(vaultPath, id);
      // O último item do histórico é o que o projeto fez por último — é o que
      // interessa ver de relance no índice.
      const last = note.history.at(-1) ?? "";
      const summary = last.replace(/^\[\[[^\]]+\]\]\s*—\s*/, "").trim();
      lines.push(`- [[${id}]]${summary ? ` — ${summary}` : ""}`);
    }
    lines.push("");
  }

  const path = join(vaultPath, "projetos.md");
