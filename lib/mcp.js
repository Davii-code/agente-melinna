...
async function toolStages(_root, args) {
  const { STAGES } = await import("./agent/stages.js");
  const lines = ["Etapas do pipeline da Melinna (`melinna run`):", ""];
  for (const stage of STAGES) {
    const flags = [stage.readOnly ? "leitura" : "escrita", stage.gate ? "PORTÃO" : null].filter(Boolean).join(", ");
    lines.push(`${stage.id} [${flags}] — ${stage.goal.split(".")[0]}.`);
    if (stage.skills.length) lines.push(`  método: ${stage.skills.join(", ")}`);
    if (stage.speckit) lines.push(`  artefato: ${stage.speckit}`);
  }
  lines.push(
    "",
    "Um portão que reprova interrompe o pipeline — não é aviso.",
    "No terminal: `melinna run \"<tarefa>\"`. Aqui dentro, você já é o executor: siga as etapas você mesmo.",
  );
  return text(lines.join("\n"));
}

async function toolRuns(_root, args) {
  const { listRuns, loadState } = await import("./agent/envelope.js");
  const runs = listRuns();
  if (runs.length === 0) return text("Nenhuma execução do modo agente registrada.");

  const lines = ["Execuções do modo agente:", ""];
  for (const id of runs.slice(0, args.limit ?? 10)) {
    const state = loadState(id);
    if (!state) continue;
    lines.push(
      `${id} — ${state.status} | US$ ${(state.spentUsd ?? 0).toFixed(4)} | ${state.stages?.length ?? 0} etapa(s)`,
    );
...
export function buildTools(root) {
  return [
    ...
    },
    {
      name: "melinna_stages",
      description:
        "Mostra as etapas do pipeline da Melinna e o método de cada uma (spec-kit + superpowers). " +
        "Use quando o usuário quiser seguir um processo disciplinado — entender, especificar, " +
        "planejar, implementar com TDD, revisar, verificar — em vez de partir direto para o código.",
      inputSchema: { type: "object", properties: {} },
      run: (args) => toolStages(root, args),
    },
    {
      name: "melinna_runs",
      description:
        "Lista as execuções do modo agente (`melinna run`) com status e custo. Use quando o " +
        "usuário perguntar o que a Melinna executou sozinha ou quanto gastou.",
      inputSchema: {
        type: "object",
        properties: { limit: { type: "number", description: "Quantas execuções listar (padrão: 10)." } },
      },
      run: (args) => toolRuns(root, args),
    },
    {
