 ...
 * @param {{ maxBytes?: number, includeReferences?: boolean }} [opts]
 *   `includeReferences: false` devolve só o SKILL.md — é o corte usado pelos
 *   perfis de economia, e vale cerca de um terço do custo do pacote.
 * @returns {string}
 */
export function readSkillBundle(skill, opts = {}) {
  const maxBytes = opts.maxBytes ?? 48 * 1024;
  let content = readFileSync(skill.path, "utf-8");
  if (opts.includeReferences === false) return content;
...
export function selectSkills(skills, tags, opts = {}) {
  ...
  return chosen
    .slice(0, limit)
    .sort((a, b) => b.score - a.score || a.skill.id.localeCompare(b.skill.id))
    .map((s) => ({ ...s.skill, score: s.score, role: s.role }));
}

/**
 * A escolha foi por casamento direto de stack, ou só pelo bônus genérico?
 *
 * O casamento é lexical: compara as tags detectadas com id, `triggers` e
 * descrição. Funciona quando a skill se nomeia pela tecnologia — o caso comum —
 * e erra calado quando não. Isto dá o sinal que faltava: uma escolha em que
 * nenhuma skill pontuou por stack quase certamente não é a melhor disponível.
 *
 * Pontuação: 10 por acerto no id, 5 nos triggers, 1 na descrição; arquitetura e
 * revisão ganham 6 e 5 sem casar com stack nenhuma. Acima de 10, houve acerto
 * direto.
 *
 * @param {Array<object>} chosen saída de selectSkills
 * @param {string[]} tags stacks detectadas
 * @returns {{ level: "forte" | "fraco" | "nenhum", direct: number, reason: string }}
 */
export function selectionConfidence(chosen, tags) {
  if (chosen.length === 0) {
    return {
      level: "nenhum",
      direct: 0,
      reason: tags.length
        ? "Stack detectada, mas nenhuma skill dela está instalada."
        : "Nenhuma stack reconhecida neste diretório.",
    };
  }

  const direct = chosen.filter((s) => (s.score ?? 0) >= 10).length;
  if (direct > 0) {
    return { level: "forte", direct, reason: `${direct} skill(s) casaram diretamente com a stack.` };
  }
  return {
    level: "fraco",
    direct: 0,
    reason:
      "Nenhuma skill casou com a stack — as escolhidas entraram pelo bônus de arquitetura/revisão. " +
      "Pode faltar instalar as da tecnologia deste projeto.",
  };
}
