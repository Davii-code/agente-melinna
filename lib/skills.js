...
export function selectSkills(skills, tags, opts = {}) {
  ...
  // Arquitetura e revisão são sempre validadas: reservamos uma vaga para cada
  // uma antes de preencher o resto por pontuação. Sem isso, um projeto com
  // muitas skills próprias (Fluig, ADVPL) enche o limite e a checagem
  // arquitetural nunca chega ao prompt.
  const chosen = [];
  for (const role of ["architecture", "review"]) {
    const best = eligible.find((s) => s.role === role);
